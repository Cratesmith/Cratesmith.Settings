...
*/ 
    ...
    /// </summary>
    /// 
    public abstract class SettingsAsset<TSelfType> : SettingsAssetBase where TSelfType:SettingsAssetBase
    {
        private static TSelfType s_instance;
	
        [System.Serializable]
        public abstract class SettingsReference : SettingsReference<TSelfType> {}
    ...
    public abstract class SettingsAssetBase : ScriptableObject
    {
        ...
        protected static void BuildDefaultAsset(Type scriptClass)
        {
            ...
            bool found = false;
            var assetFilename = GetAssetFileName(scriptClass);
            var prevAssets = AssetDatabase.FindAssets(Path.GetFileNameWithoutExtension(assetFilename));
            foreach(var prevAssetGuid in prevAssets)
            {
                var prevAssetPath = AssetDatabase.GUIDToAssetPath(prevAssetGuid);
                ...
                var prevAsset = AssetDatabase.LoadAssetAtPath(prevAssetPath, scriptClass);
                if(prevAsset!=null && !found)
                {
                    AssetDatabase.MoveAsset(prevAssetPath, assetPath);
                }
                else 
                {
